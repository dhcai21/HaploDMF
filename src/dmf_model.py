...
#        read_all = torch.sqrt(torch.sum(read_vector**2,dim=1))
#        snv_all = torch.sqrt(torch.sum(snv_vector**2,dim=1))
#        y = torch.divide(torch.sum(read_vector*snv_vector,dim=1),torch.multiply(read_all,snv_all))
