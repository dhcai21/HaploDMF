...
python ./src/fre_matrix.py $file_bam_sorted $file_snv $file_prefix $cond_pro $smallest_snv $num_read_1 $num_read_2 $gap $fre_snv $thread
...
python ./src/dmf.py --i $file_prefix"_matrix.pickle" --o $file_path --p $prefix --w $weight --l $lr --n $ep --algorithm $alg --c1 $c1 --c2 $c2 --largest_num $li
...
python ./src/out_haplotypes.py $prefix $file_bam_sorted $file_path $file_ref $s_pos $e_pos
